    ...
    try {
        const response = await fetch('https://fairlane-189762133025.us-central1.run.app/query/?query=' + query, {
            ...
            if (data.metadata) {
                ...
                loadPDF(docsMap.get(data.metadata.filename), data.metadata.page_number); 
...
async function fetchDocuments() {
    ...
    try {
        const response = await fetch('https://fairlane-189762133025.us-central1.run.app/get_docs'); // Replace with your actual endpoint
        if (!response.ok) {
        ...
        data.source_docs.forEach(doc => {
            ...
            .on('click', function() {
                loadPDF(doc.url); // Load the PDF when clicked
            });
            const iconDiv = $('<div></div>')

            const iconLink = $('<a></a>')
            .attr('href', doc.url) // Set the URL to the document
